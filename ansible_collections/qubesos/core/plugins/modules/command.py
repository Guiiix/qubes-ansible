...
def register_command(
    ...
    def decorator(func):
        @wraps(func)
        def wrapper(module, qubes_virt: QubesVirt):
            if wrapper.__command_type__ == CommandType.VM:
                guest = get_module_param(module, "guest")
...
def create_inventory(module, qubes_virt: QubesVirt):
    """
...
def create_qube(module, qubes_virt: QubesVirt, guest: str):
    try:
        ...
        qubes_virt.create(
            ...
            get_module_param(module, "label", False),
...
def remove_tags(module, qubes_virt: QubesVirt, guest: str):
    vm = qubes_virt.get_vm(guest)
    changed = False
    tags = get_module_param(module, "tags", False)
    if not tags:
        module.fail_json(rc=1, msg={"Error": "Missing tag(s) to remove."})
...
@register_command("stop", CommandType.VM)
